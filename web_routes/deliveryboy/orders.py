...
def delete_order(order_id):
    try:
        ...
        if result:
            ...
            logger.warning(f"[ORDERS] No result returned from '{procedure_name}' for order_id={order_id}, user {user_id}")
...
def get_order_commodities(order_id):
    try:
        ...
        logger.info(f"[ORDERS] Procedure 'get_order_commodities' executed with params={params}, results_count={len(results) if results else 0}")
...
def get_order_details_by_status():
    try:
        ...
        logger.info(f"[ORDERS] User {user_id} fetching orders by status={order_status}, order_id={order_id}, delivery_date={delivery_date}, page={page_number}, size={page_size}")

        results = db.call_procedure("getOrderDetailsByStatus", params)
        logger.info(f"[ORDERS] Procedure 'getOrderDetailsByStatus' executed with params={params}, results_count={len(results) if results else 0}")
...
def get_orders_by_employee(employee_id):
    try:
        ...
        logger.info(f"[ORDERS] Procedure '{procedure_name}' executed with params={params}, results_count={len(result) if result else 0}")
...
def get_order_details_bystage():
    try:
        ...
        results = db.call_procedure(procedure_name, params)
        logger.info(f"[ORDERS] Procedure '{procedure_name}' executed with params={params}, results_count={len(results) if results else 0}")
...
def get_order_summary():
    try:
        ...
        logger.info(f"[ORDERS] Procedure '{procedure_name}' executed with params={params}, result_count={len(result) if result else 0}")
...
def get_order_delivery_details():
    try:
        ...
        if page_number < 1 or page_size < 1:
            logger.warning(f"[ORDERS] User {user_id} provided invalid pagination: page_number={page_number}, page_size={page_size}")
            return jsonify({
                "status": "Failure",
...
def getdeliverorderdetails():
    try:
        ...
        logger.info(f"[ORDERS][getdeliverorderdetails][{employee_id}] Successfully fetched deliver order details for orderId={order_id}")
...
def insert_documents():
    try:
        ...
        logger.info(f"[ORDERS][insert_documents][{employee_id}] Request with orderId={order_id}, doc_type={doc_type}, files_count={len(files)}")
...
def get_documents_by_order_and_category():
    try:
        ...
        logger.info(f"[ORDERS][get_documents_by_order_and_category] Request with orderId={order_id}, category={category}")
        ...
        if not result:
            logger.info(f"[ORDERS][get_documents_by_order_and_category] No documents found for orderId={order_id}, category={category}")
            return jsonify({"status": "Failure", "data": result}), 200

        logger.info(f"[ORDERS][get_documents_by_order_and_category] Successfully fetched {len(result)} documents for orderId={order_id}, category={category}")
...
def updatedeliverorder():
    try:
        ...
        logger.info(f"[ORDERS][updatedeliverorder][{employee_id}] Update request for orderId={order_id}, orderStatus={order_status}, reasonId={reason}")
...
def get_draft_order_details():
    try:
        ...
        if page_number < 1 or page_size < 1:
            logger.warning(f"[ORDERS][get_draft_order_details][{employee_id}] Invalid pagination values: pageNumber={page_number}, pageSize={page_size}")
        ...
        result = db.call_procedure(procedure_name, params)
        logger.info(f"[ORDERS][get_draft_order_details][{employee_id}] Procedure '{procedure_name}' executed, result length: {len(result)}")
        ...
        order_rows = order_result if isinstance(order_result, list) else ([order_result] if isinstance(order_result, dict) else [])

        logger.info(f"[ORDERS][get_draft_order_details][{employee_id}] Successfully fetched draft orders, count={len(order_rows)}")
        return jsonify({
            "data": {
                "orders": order_rows,
                "currentPage": page_number,
