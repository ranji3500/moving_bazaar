...
INSERT INTO `outstanding_balance` VALUES (1,10,1068025,150.50,'2025-04-03 21:47:38'),(2,10,1068025,150.50,'2025-04-03 21:48:36'),(3,10,1068025,150.50,'2025-04-03 21:58:52'),(4,10,1068025,150.50,'2025-04-03 22:21:48');
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetOrderDetailsbyStagewise`(IN p_order_id BIGINT, IN p_stage VARCHAR(20))
BEGIN
    -- Order Stage
...
BEGIN
    -- Retrieve order details including sender, receiver, and summary
    SELECT 
        o.order_id,
        sender.store_name AS sender_name,
        sender.outstanding_price AS sender_outstanding_price,
        receiver.store_name AS receiver_name,
        receiver.outstanding_price AS receiver_outstanding_price,
        c.item_name,
        oi.quantity,
        c.price,
        (oi.quantity * c.price) AS total_price,
        (SELECT SUM(oi2.quantity * c2.price) 
         FROM moving_bazaar.order_items oi2
         JOIN moving_bazaar.commodity c2 ON oi2.commodity_id = c2.commodity_id
         WHERE oi2.order_id = p_order_id) AS total_order_price,
        ((SELECT SUM(oi2.quantity * c2.price) 
          FROM moving_bazaar.order_items oi2
          JOIN moving_bazaar.commodity c2 ON oi2.commodity_id = c2.commodity_id
          WHERE oi2.order_id = p_order_id) + sender.outstanding_price) AS grand_total
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM moving_bazaar.outstanding_balance WHERE customer_id = p_customer_id) THEN
...
BEGIN
    ...
    ELSE
        -- Insert record
        INSERT INTO moving_bazaar.outstanding_balance (customer_id, order_id, outstanding_amount)
        VALUES (p_customer_id, p_order_id, p_outstanding_amount);
...
-- Dump completed on 2025-04-03 22:33:34
