...
CREATE TABLE `orders` (
  ...
  `reason` enum('payment_failed','out_of_stock','invalid_address','delivery_failed','other') DEFAULT NULL,
...
INSERT INTO `orders` VALUES (1068025,13,14,10,'In Transit','order','2025-03-01 01:31:06','2025-04-27 17:20:52',NULL),(1454275,13,14,10,'In Transit','order','2025-03-01 01:31:45','2025-04-27 17:20:52',NULL),(3342978,13,14,10,'In Transit','order','2025-03-01 01:33:34','2025-04-27 17:20:52',NULL),(5135981,13,14,10,'In Transit','order','2025-03-01 01:35:13','2025-03-01 01:35:13','other'),(16077556,12,13,10,'In Transit','order','2025-03-16 07:46:07','2025-03-16 07:46:07','other'),(17455902,12,13,10,'In Transit','order','2025-03-16 07:47:45','2025-03-16 07:47:45','other'),(19280588,12,13,10,'In Transit','order','2025-03-16 07:49:28','2025-03-16 07:49:28','other'),(55030183,12,13,10,'In Transit','order','2025-03-01 01:25:03','2025-03-01 01:25:03','other'),(55066636,12,13,10,'In Transit','order','2025-03-01 01:25:06','2025-03-01 01:25:06','other'),(56012634,12,13,10,'In Transit','order','2025-03-01 01:26:01','2025-03-01 01:26:01','other'),(58263727,13,14,10,'In Transit','order','2025-03-01 01:28:26','2025-03-01 01:28:26','other'),(58430656,13,14,10,'In Transit','order','2025-03-01 01:28:43','2025-03-01 01:28:43','other'),(59050656,13,14,10,'In Transit','order','2025-03-01 01:29:05','2025-03-01 01:29:05','other');
...
INSERT INTO `outstanding_balance` VALUES (6,10,3342978,150.50,'2025-04-05 22:29:36','close','2025-04-27 15:12:22');
...
BEGIN
    ...
    ELSE
        SELECT 
            'No documents found' AS message,
            FALSE AS status,
            inOrderId AS orderId,
            inCatagory AS catagory;
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
    ...
    -- Get order info
    SELECT sender_id, receiver_id, order_status
    INTO vSenderId, vReceiverId, vOrderStatus
    FROM orders 
    WHERE order_id = inputOrderId;

    -- Get sender store name
    SELECT store_name
    ...
    -- Final result: document info with JSON paths and related order/store info
    SELECT
        vSenderId AS senderId,
        vReceiverId AS receiverId,
        vOrderStatus AS orderStatus,
        vSenderStoreName AS senderStoreName,
        vReceiverStoreName AS receiverStoreName,
        d.doc_id AS docId,
        d.catagory AS category,
        JSON_ARRAYAGG(dp.path) AS paths
    FROM documents d
...
BEGIN
    ...
    -- Create a temporary table to store inserted document IDs
    CREATE TEMPORARY TABLE temp_doc_ids (
        document_id INT
    );
    ...
    WHILE i < pathCount DO
        ...
        INSERT INTO temp_doc_ids (document_id) VALUES (docId);

        SET i = i + 1;
    END WHILE;

    -- Final output: one row per inserted document
    SELECT 
        'Documents inserted successfully' AS message,
        TRUE AS status,
        document_id AS documentId,
        inOrderId AS orderId
    FROM temp_doc_ids;

    -- Clean up (optional in most engines, but good practice)
...
-- Dump completed on 2025-05-01 22:30:34
